...
/* =========================================================
   FREEBIE-PAGE LOGIN LOG
   Records that a signed-in user was seen on the public
   Freebie page, so the owner dashboard can show "who logged
   in from the freebie page" as its own list. Logged once per
   browser session (not on every reload) via sessionStorage.

   FIX: the session flag is now scoped per-user
   ("tusdioFreebieLoginLogged_<uid>") instead of one shared
   global key. Previously, if User A logged in and then User B
   logged in later in the *same* browser tab/session (e.g. a
   shared or public computer), B's login would silently never
   get recorded — A's flag was already set, so the early-return
   above skipped B entirely. Scoping the flag to each uid means
   every distinct signed-in user gets logged once per session,
   regardless of who else used that browser before them.
   ========================================================= */

const FREEBIE_LOGIN_SESSION_PREFIX = "tusdioFreebieLoginLogged_";

async function logFreebieLogin(user) {

    if (!user) return;

    const flagKey = FREEBIE_LOGIN_SESSION_PREFIX + user.uid;
    if (sessionStorage.getItem(flagKey)) return;

    try {
        await addDoc(collection(db, "freebie_logins"), {
            uid: user.uid,
        ...
        console.error("Failed to log freebie page login:", error);
    }
...
/* =========================================================
   FREEBIE-PAGE DOWNLOAD LOG
   Records every time a signed-in user actually taps
   "Get this resource" on a freebie, so the owner dashboard
   can show real download activity (not just page logins),
   per resource, in its own "freebie_downloads" collection.
   ========================================================= */

async function logFreebieDownload(user, freebie) {

    if (!user || !freebie) return;

    try {
        await addDoc(collection(db, "freebie_downloads"), {
            uid: user.uid,
            name: user.displayName || (user.email ? user.email.split("@")[0] : "User"),
            email: user.email || "",
            freebieTitle: freebie.title || "",
            freebieCategory: (freebie.tags && freebie.tags[0]) || "Freebie",
            downloadLink: freebie.download || "",
            createdAt: serverTimestamp()
        });
    } catch (error) {
        console.error("Failed to log freebie download:", error);
    }
...
function setupDownloadGate() {
    ...
    if (btn) {
        btn.addEventListener("click", (event) => {
            ...
            if (currentUser) {
                // signed in — log the download, then let the link open normally
                if (currentIndex !== null && freebies[currentIndex]) {
                    logFreebieDownload(currentUser, freebies[currentIndex]);
                }
                return;
            }
...
const freebies = [
    ...
    { title: "Free Softcover Mockup on Leather Chair", img: "https://d2pas86kykpvmq.cloudfront.net/uploads/softcover_books_preview_2_50e61ddf83.avif", extraImg: "", tags: ["Magazines"], download: "https://wannathis.one/mockups/softcover-mockup-on-leather-chair?from=mockups-free" },
    { title: "Free Paper and Business Card Mockup with Paper Clip", img: "https://ls-graphics-directus-direct.b-cdn.net/assets/808a078c-f471-4bad-9c1e-f510d38b2f9a?width=1700&format=webp&quality=90", extraImg: "", tags: ["Magazines"], download: "https://www.ls.graphics/assets/free-paper-and-business-card-mockup-with-paper-clip" },
    ...
    { title: "Free Metal Billboard Mockup", img: "https://cdn.cosmos.so/8f5024ea-2a18-401d-85cf-aa13fed0fa8b?format=webp", extraImg: "", tags: ["BillBoards"], download: "https://www.mockupcloud.com/free/metal-billboard-mockup" },
    { title: "Free Digital Screen Advertising Mockup", img: "https://cdn.cosmos.so/bd6218ce-5df7-4255-9f0b-ac475acf6692?format=webp&w=2048", extraImg: "", tags: ["BillBoards"], download: "https://www.mockupcloud.com/free/digital-screen-advertising-mockup" },
...
/* =========================================================
   SHARE URL BUILDER
   NOTE: This is the ONLY function changed from the original file.
   It now points to a server-renderable path (/freebie/share?fid=N)
   instead of the SPA hash (#freebie-N), because link-preview bots
   (WhatsApp, Twitter, Facebook, etc.) never read anything after "#".
   Pair this with a Cloud Function (or equivalent) that serves
   per-item Open Graph tags at that path, then redirects real
   visitors into the app via the hash as before.
   ========================================================= */

const SHARE_BASE_URL = "https://tusdio.online/freebie/share";

function buildShareUrl(index) {
    // Path-based, not a query param — GitHub Pages serves files by
    // path (freebie/share/<index>/index.html), it can't read query
    // strings server-side to decide what to return.
    return `${SHARE_BASE_URL}/${index}/`;
}
...
function updateShareLinks(index) {
    ...
    // CHANGED: use the server-renderable share URL (query param, not hash)
    // so WhatsApp / Twitter / Facebook / etc. bots pick up per-item
    // og:title / og:description / og:image instead of the site default.
    const url = buildShareUrl(index);
